...
class Storage extends Home_Controller{
	...
	public function index() {
		redirect('/Storage/requests/1', 'refresh');
	}

	public function requests($page = 1, $recordsInPage = 5, $pageRange = 5) {
		...
		*/
		...
		$paginationHTML .= "<li><a href='" . site_url("Storage/requests/" . max(1, $page - 1)) . "' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a></li>";
		...
		for ($i=0; $i<$pageRange; $i++) {
			...
			$paginationHTML .= "<li><a class='" . $className . "' href='" . site_url("Storage/requests/" . $currentPage) . "'>" . $currentPage . "</a></li>";
		}

		$paginationHTML .= "<li><a href='" . site_url("Storage/requests/" . min($pageCnt, $page + 1)) . "' aria-label='Previous'><span aria-hidden='true'>&raquo;</span></a></li>";
		...
		foreach ($requests as $key=>$req) {
			...
			$link = site_url("/Storage/request_edit/" . $req->id);
		...
		parent::initView('storage', 'storage', 'Manage media such as images and videos',
			array(
	...
	public function request_edit($id) {
		...
		parent::initView('request_edit', 'storage', 'Manage media such as images and videos',
			array(
				"request" => $req,
